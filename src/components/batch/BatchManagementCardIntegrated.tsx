...
export function BatchManagementCardIntegrated({ 
  tripId, 
  tripDate, 
  onCreateBatch, 
  onViewBatch 
}: BatchManagementCardIntegratedProps) {
  const { data: batches = [], isLoading } = useShipmentBatches(tripId);
    ...
    return (
      <Card>
        <CardContent className="py-4">
          <div className="text-center text-gray-500">Cargando bultos...</div>
        </CardContent>
      </Card>
    );
  }
        ...
        {batches.length === 0 ? (
          <div className="text-center py-6 bg-gray-50 rounded-lg">
            ...
            <p className="text-sm text-gray-500 mb-1">
              No hay bultos creados para este viaje
            </p>
            <p className="text-xs text-gray-400">
              Crea bultos para organizar las encomiendas en contenedores físicos
            </p>
          </div>
        ) : (
