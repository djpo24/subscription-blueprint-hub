...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'packages' AND column_name = 'deleted_by'
  ) THEN
    ALTER TABLE public.packages 
...
CREATE INDEX IF NOT EXISTS idx_packages_deleted_at ON public.packages(deleted_at) WHERE deleted_at IS NOT NULL;
...
DROP POLICY IF EXISTS "Authenticated users can access active packages" ON public.packages;
DROP POLICY IF EXISTS "Authenticated users can manage active packages" ON public.packages;
DROP POLICY IF EXISTS "Admins can view all packages including deleted" ON public.packages;
DROP POLICY IF EXISTS "Admins can restore deleted packages" ON public.packages;
